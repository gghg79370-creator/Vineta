...
export const ReviewsSection: React.FC<ReviewsSectionProps> = ({ reviews, onWriteReviewClick }) => {
    ...
    return (
        <div className="py-8" id="reviews">
            <div className="flex flex-col md:flex-row-reverse gap-10 lg:gap-16 items-start">
                <div className="w-full md:w-1/3 flex-shrink-0">
                    <h3 className="text-xl font-bold mb-2">تقييم العملاء</h3>
                    {totalReviews > 0 ? (
                        <>
                            ...
                            <div className="space-y-1.5">
                                {Object.entries(ratingSummary).reverse().map(([star, count]) => (
                                    <div key={star} className="flex items-center gap-2 text-sm">
                                        ...
                                        <span className="w-8 text-right text-brand-text-light">{count}</span>
                                    </div>
                                ))}
                            </div>
                        </>
                     ...
                     <button onClick={onWriteReviewClick} className="mt-6 bg-brand-dark text-white font-bold py-3 px-8 rounded-full hover:bg-opacity-90 w-full">اكتب تقييمًا</button>
                </div>
                                ...
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex flex-col sm:flex-row justify-between sm:items-center">
                                          ...
                                          <div className="flex-shrink-0 mt-2 sm:mt-0">{renderStars(review.rating)}</div>
                                        </div>
                                        <p className="text-brand-text leading-relaxed mt-2">{review.text}</p>
                                    </div>
                                </div>
                            </div>
                        )) : <p className="text-gray-500 py-8 text-center">كن أول من يكتب تقييماً لهذا المنتج.</p>}
                    </div>
                </div>
            </div>
        </div>
