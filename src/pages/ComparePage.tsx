...
const ComparePage = ({ navigateTo }: ComparePageProps) => {
    ...
    const renderStars = (rating = 5) => (
        <div className="flex justify-center">
            {[...Array(5)].map((_, i) => (
                <StarIcon key={i} className={`w-4 h-4 ${i < rating ? 'text-yellow-400' : 'text-brand-border'}`} />
            ))}
        </div>
    );
        ...
        <div className="bg-brand-subtle">
            ...
            <div className="container mx-auto px-4 py-12">
                {productsToCompare.length > 0 ? (
                    <div className="overflow-x-auto bg-surface p-4 rounded-lg shadow-sm border border-brand-border">
                            ...
                            <tbody>
                                <tr className="border-b border-brand-border">
                                    ...
                                    {productsToCompare.map(p => (
                                        <td key={p.id} className="p-2 text-center w-1/5 relative">
                                            <button onClick={() => handleRemove(p.id)} className="absolute top-2 right-2 p-1 bg-brand-bg/50 rounded-full hover:bg-brand-bg z-10"><TrashIcon size="sm" className="text-brand-text-light hover:text-brand-sale"/></button>
                                ...
                                <tr className="border-b border-brand-border">
                                 ...
                                 <tr className="border-b border-brand-border bg-brand-subtle/50">
                                    ...
                                    {productsToCompare.map(p => (
                                        <td key={p.id} className="p-4 text-center font-semibold text-brand-instock">{p.availability || 'متوفر'}</td>
                                    ))}
                                </tr>
                                 <tr className="border-b border-brand-border">
                     ...
                     <div className="text-center py-20 bg-surface rounded-lg shadow-sm border border-brand-border">
