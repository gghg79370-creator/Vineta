...
import { SparklesIcon, ArrowUpTrayIcon, ShoppingBagIcon, CameraIcon, MagicWandIcon, ArrowUturnLeftIcon } from '../components/icons';
...
const AiTryOnPage: React.FC<AiTryOnPageProps> = ({ navigateTo, addToCart }) => {
    ...
    const selectableProducts = useMemo(() => {
        ...
        const isClothing = (p: Product) => !p.tags.includes('إكسسوارات') && !p.tags.includes('حقيبة') && !p.tags.includes('ساعة') && !p.tags.includes('نظارة');
    ...
    const handleGenerate = async () => {
        if (!userImage.file || !selectedProduct) {
            ...
            addToast('الرجاء تحميل صورة واختيار منتج.', 'error');
            return;
        }
        setIsLoading(true);
        setError('');
                     ...
                     <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
                        ...
                        <div className="aspect-[4/5] bg-brand-subtle rounded-2xl border border-brand-border/50 shadow-lg flex flex-col items-center justify-center p-4 relative overflow-hidden">
                            ...
                            {generatedImage && userImage.preview ? (
                                <ImageComparisonSlider beforeImage={userImage.preview} afterImage={generatedImage} />
                            ) : userImage.preview ? (
                                ...
                                <div className="text-center">
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                                        ...
                                        <button onClick={() => setIsCameraActive(true)} className="w-full sm:w-auto bg-surface border border-brand-border text-brand-dark font-bold py-3 px-6 rounded-full flex items-center justify-center gap-2 hover:bg-brand-subtle"><CameraIcon/> <span>استخدم الكاميرا</span></button>
                                    </div>
                                </div>
                            )}
                             <input type="file" ref={fileInputRef} onChange={handleFileSelect} accept="image/*" className="hidden" />
                             {userImage.preview && (
                                <button onClick={handleReset} className="absolute bottom-4 left-4 bg-brand-bg/50 backdrop-blur-sm text-brand-dark font-semibold py-2 px-4 rounded-full text-sm shadow-md hover:bg-brand-bg z-10 flex items-center gap-2">
                                    <ArrowUturnLeftIcon size="sm" />
                                    <span>إعادة تعيين</span>
                                </button>
                             )}
                              <p className="absolute bottom-4 text-xs text-brand-text-light/80 z-10 text-center w-full px-12 pointer-events-none">
